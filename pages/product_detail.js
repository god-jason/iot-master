  ...
  methods: {
    add_tab_settings() {
      if (this.data.configurable) {
        this.content.tabs.push({ title: '配置参数', page: 'product_setting_setting', params: { id: this.params.id } })
      }
    },
    add_tab_actions() {
      if (this.data.controllable) {
        this.content.tabs.push({ title: '远程操作', page: 'product_setting_action', params: { id: this.params.id } })
      }
    },
    add_tab_parameters() {
      if (this.data.writable) {
        this.content.tabs.push({ title: '修改变量', page: 'product_setting_parameter', params: { id: this.params.id } })
      }
    },
    add_tab_validators() {
      if (this.data.validate) {
        this.content.tabs.push({ title: '数值检查', page: 'product_setting_validator', params: { id: this.params.id } })
      }
    },
    add_tab_versions() {
      if (this.data.ota) {
        this.content.tabs.push({ title: '固件版本', page: 'version', params: { product_id: this.params.id } })
      }
    }
  }
}