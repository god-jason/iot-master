...
return {
  ...
  fields: [
    ...
    { key: 'port', label: '端口', type: 'number', default: 1 },
    {
      key: 'protocol',
      label: '协议',
      type: 'select',
  ...
  submit_api: 'table/socket/update/:id/:gateway_id',
  // 页面挂载时执行
  mount() {
    this.load_protocols()
  },
  methods: {
    load_protocols() {
      this.request.get('protocol/list').subscribe(res => {
        this.content.fields[6].options = [{ value: '', label: '无' }].concat(
          res.data.map(d => {
    ...
    load_protocol_option() {
      if (this.editor.value.protocol) {
        this.request.get('protocol/' + this.editor.value.protocol).subscribe(res => {
          this.content.fields[7].children = res.data.option_fields
          setTimeout(() => this.editor.rebuild(), 200)
        })
      } else {
        this.content.fields[7].children = []
        setTimeout(() => this.editor.rebuild(), 200)
      }
    }
  }
